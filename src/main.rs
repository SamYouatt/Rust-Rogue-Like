...
fn render_all(tcod: &mut Tcod, game: &mut Game, objects: &[Object], fov_recompute: bool) {
    ...
    // sort non blocking objects to top
...
fn monster_death(monster: &mut Object, game: &mut Game) {
    game.messages.add(format!("You killed the {}!", monster.name), ORANGE);
    monster.char = '%';
...
fn move_towards(id: usize, target_x: i32, target_y: i32, map: &Map, objects: &mut[Object]) {
    // distance from this object to target
    let dx = target_x - objects[id].x;
    let dy = target_y - objects[id].y;
    let distance = ((dx.pow(2) + dy.pow(2)) as f32).sqrt();

    // normalise vector to 1N and round to allow movement within grid
...
fn main() {
    ...
    game.messages.add(
        "Enter the dungeon and try to survive!",
        RED,
    );
